...
$bdd = new PDO('mysql:host=mysql-databasetest.alwaysdata.net;dbname=databasetest_promoteddb;', '335454', 'Tigrou2023');
    if(!empty($_GET['u'])){
        $password_reset_token = htmlspecialchars(base64_decode($_GET['u']));
        $check = $bdd->prepare('SELECT * FROM USERS WHERE IDUsers = ?');
        $check->execute(array($password_reset_token));
        $row = $check->rowCount();
...
<html lang="fr">
  <head>
    <title>Reinitialiser mon mot de passe</title>
    <!-- Required meta tags -->
    <meta charset="utf-8">
        ...
        <div class="container">
          <div class="col-11">
              <div class="card text-center m-4 shadow p-3 mb-5 bg-white rounded">
                <div class="card-body">
                  <h4 class="card-title p-3">Reinitialiser mon mot de passe</h4>
                    <div class="form-group">
                        <form action="password_change_post.php" method="POST">
                            <input type="hidden" name="password_reset_token" value="<?php echo (base64_decode(htmlspecialchars($_GET['u']))); ?>"  />
